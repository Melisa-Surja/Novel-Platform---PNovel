<template>
    <div class="p-4 border-2 border-dashed rounded-md opacity-50 select-none pointer-events-none text-sm flex items-center justify-center">
        <div class="text-center">
            <svg class="w-16 h-16 mx-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="0.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            <template v-if="!$slots.default">Image Not Found</template>
            <slot v-else></slot>
        </div>
    </div>
</template>

<script>
export default {
  props: {
    // _class: String,
  }
}
</script>
